...
//Activer les DMA
	...
	*(A5 + DMACON) = 0x83C0; 	//DMAEN=1, COPEN=1, BPLEN=1, COPEN=1, BLTEN=1
...
//Surface du plasma (simple rectangle de couleur 1)
	...
	WAITBLIT();
	setCopper_w(BLTDAT , 0xFFFF);
	setCopper_w(BLTMOD , 0);
...
uint32 rgbOffsets[]={ 0 };
uint32 rowOffsets[]={ 0 };
uint32 copperList0[]={ 0 };
uint32 copperList1[]={ 0 };
uint16 dmacon[]={ 0 };
uint16 intena[]={ 0 };
uint16 intreq[]={ 0 };
